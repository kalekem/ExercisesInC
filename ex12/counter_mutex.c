...
Shared *make_shared (int end)
{
    ...
    //the initial value of semaphore is set to 1 in order to allow the first thread to run
...
void child_code (Shared *shared)
{
    ...
    while (1) {
        //blocks processes until the other thread completes using the shared variable
        sem_wait(shared->sem);

        if (shared->counter >= shared->end) {

            sem_signal(shared->sem); //signals the waiting thread to run once the other thread is done 
            return;
        }
        ...
        //the value of the sem is incremented for other threads to run
